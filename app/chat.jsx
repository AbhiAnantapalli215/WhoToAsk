...
// --- MODIFIED: This function now calls the /query endpoint ---
  const triggerAIResponse = async (text, sessionId) => {
    try {
      ...
      await addDoc(collection(db, 'users', user.uid, 'sessions', sessionId, 'messages'), {
        text: 'Query failed. Please check your connection and try again.',
        sender: 'ai',
        timestamp: serverTimestamp(),
      });
    }
...
// --- NEW: Function for the document/image upload flow ---
  const runDocumentFlow = async (initialText, sessionId) => {
    try {
      // 1. Call /summarize. The backend already has the context from /upload
      const summaryRes = await fetch(`${BACKEND_URL}/summarize`, { method: 'POST' });
      const summaryData = await summaryRes.json();
      const summary = summaryData.summary || 'Could not generate a summary.';
      ...
      // 2. Call /find_lawyers. This also uses the backend's summary context
      const lawyersRes = await fetch(`${BACKEND_URL}/find_lawyers`, { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ summary: summary }) // Send summary as per backend example
      });
      const lawyerData = await lawyersRes.json();
      let lawyerText = 'No lawyers found matching this case.'; // Default text

      if (lawyerData.lawyers && lawyerData.lawyers.length > 0) {
        lawyerText = 'Based on this, I found some lawyers who might be able to help:\n\n';
        lawyerText += lawyerData.lawyers.map(l => {
          return `• ${l.name || 'N/A'}\n  Phone: ${l.phone || '-'}\n  Email: ${l.email || '-'}`;
        }).join('\n\n');
      }

      // Add lawyer info to chat
      await addDoc(collection(db, 'users', user.uid, 'sessions', sessionId, 'messages'), {
        text: lawyerText,
        sender: 'ai',
        timestamp: serverTimestamp(),
      ...
      // 3. Finally, call /query to get a direct answer to the user's initial message
      await triggerAIResponse(initialText, sessionId);

    } catch (e) {
      console.error("Document flow failed:", e);
      await addDoc(collection(db, 'users', user.uid, 'sessions', sessionId, 'messages'), {
        text: 'The document analysis flow failed. Please ask your question directly.',
        sender: 'ai',
        timestamp: serverTimestamp(),
      });
    }
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      if (params.sessionId) {
        ...
        // 2. Check for an initial message
        if (params.initialMessage) {
          // The initialMessage now contains the REAL extracted text from index.jsx
          const initialText = params.initialMessage;

          // 3. Send the initial user message
          const userMessage = {
            text: initialText,
            sender: 'user',
            timestamp: serverTimestamp(),
          ...
          // --- MODIFICATION: Use the correct flow ---
          // 4. Trigger the correct AI flow
          if (sessionType === 'document' || sessionType === 'photo') {
            // Use the multi-step flow for documents/images
            await runDocumentFlow(initialText, newId);
          } else {
            // Use the standard /query flow for plain text chats
            await triggerAIResponse(initialText, newId);
          }
          // --- END MODIFICATION ---
        }
